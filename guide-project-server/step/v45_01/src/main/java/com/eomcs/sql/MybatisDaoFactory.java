...
public class MybatisDaoFactory {
  ...
  public MybatisDaoFactory(SqlSessionFactory sqlSessionFactory) {
    invocationHandler = (proxy, method, args) -> {
      ...
      // 할 일 : SqlSession을 이용하여 SQL을 실행
      // 준비 1) : 실행할 SQL의 namespace와 id
      // 준비 2) : selectList/selectOne insert/update/delete 여부를 알아야함

      // 동작 원리
      // 1) 인터페이스 명과 메서드명을 이용하여 실행할 SQL 아이디를 알아낸다.
      // 2) 리턴타입에 따라 호출할 메서드를 결정한다.
      // 3) 파라미터 유무에 따라 메서드에 넘길 항목을 결정한다.

      // SQL ID 알아내기
      Class<?> cls = proxy.getClass(); // 프록시 객체의 정보를 알아낸다.
      Class<?> daoInterface = cls.getInterfaces()[0]; // 프록시 객체가 구현한 인터페이스 정보를 알아낸다.
