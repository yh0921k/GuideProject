...
public class MemberDaoImpl implements MemberDao {
  ...
  public int insert(Member member) throws Exception {
    try (Connection con = dataSource.getConnection(); Statement stmt = con.createStatement();) {
      String sql = String.format(
          "insert into lms_member(name, email, pwd, tel, photo) values('%s', '%s', password('%s'), '%s', '%s')",
  ...
  public int update(Member member) throws Exception {
    try (Connection con = dataSource.getConnection(); Statement stmt = con.createStatement();) {
      String sql = String.format(
          "update lms_member set name='%s', email='%s', pwd=password('%s'), tel='%s', photo='%s' where member_id=%d",
          member.getName(), member.getEmail(), member.getPassword(), member.getTel(),
  ...
  public Member findByEmailAndPassword(String email, String password) throws Exception {
    try (Connection con = dataSource.getConnection(); Statement stmt = con.createStatement();) {
      String query = String.format(
          "select member_id, name, email, pwd, tel, photo from lms_member where email like '%s' and pwd like password('%s')",
          email, password);
